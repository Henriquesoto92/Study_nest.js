...
describe('AppController (e2e)', () => {
  ...
  it('Obter os dados do usuario', async () => {
    const response = await request(app.getHttpServer())
      .post('/auth/me')
  ...
  it('não conseguir ver a lista de todos os usuários, como role.user', async () => {
    const response = await request(app.getHttpServer())
      .get('/users')
      .set('Authorization', `Bearer ${accessToken}`)
      .send();
  ...
  it('Alterando manualmente o usuario para a função de administrador', async () => {
  ...
  it('tentar ver a lista de todos os usuários, como role.admin', async () => {
    const response = await request(app.getHttpServer())
      .get('/users')
      .set('Authorization', `Bearer ${accessToken}`)
      .send();
